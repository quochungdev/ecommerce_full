...
export default function ModalUpdateAddress({ addressId, a }) {
  ...
  const [homeAddress, setHomeAddress] = useState(a.address);
  const [provinces, setProvinces] = useState([]);
  const [districts, setDistricts] = useState({});

  const proviceCode = provinces != null && provinces.find((p) => p.name == a.city)?.code;
  const districtName = provinces !== null && districts.districts
  ? districts.districts.find((p) => p.name == a.ward)?.name
  : null;

const districtCode = provinces !== null && districtName != undefined
  ? districts.districts.find((p) => p.name == districtName)?.code
  : null;
  ...
  useEffect(() => {
    if (proviceCode !== undefined) {
      setSelectedProvince(proviceCode);
      setSelectedDistrict(districtCode)
      getDistrictsByProvince(proviceCode)
        .then((response) => {
