...
export default function VerifyPassword() {
  ...
  const verifyPassword = async () => {
    ...
    try {
      let res = await authApi().post(endpoints["verify_password"], formData);
      res.data === true
        ? navigate("/thong-tin-ca-nhan/change")
        : toastError("Mật khẩu không khớp");
    } catch (error) {
      toastError("Xác minh thất bại");
      console.log(error);
    }
  };
  return (
      ...
      <div className="border">
        <div className=" h-1/3 m-4">
          <div className="flex items-center">
            <div className="w-2/4">
              <Link to="/thong-tin-ca-nhan/change-password">
